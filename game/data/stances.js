const STANCES = {
  marche: {
    id: 'marche', name: 'Marche', icon: '/icons/marche.svg',
    vitesse: +1,
    attack_cac: -1, attack_tir: -2,
    defense_cac: -2, defense_tir: -1,
    puissance_cac: 0, puissance_tir: -1,
    intimidation_cac: +1, intimidation_tir: 0,
    courage_cac: -1, courage_tir: -1,
    esquive_cac: 0, esquive_tir: +2,
    precision_cac: 0, precision_tir: -2,
    armure: -1, armure_tour: 0, moral_tour: 0, vitalite_tour: 0,
  },
  combat: {
    id: 'combat', name: 'Combat', icon: '/icons/combat.svg',
    vitesse: 0,
    attack_cac: 0, attack_tir: 0,
    defense_cac: 0, defense_tir: 0,
    puissance_cac: 0, puissance_tir: 0,
    intimidation_cac: 0, intimidation_tir: 0,
    courage_cac: 0, courage_tir: 0,
    esquive_cac: 0, esquive_tir: 0,
    precision_cac: 0, precision_tir: 0,
    armure: 0, armure_tour: 0, moral_tour: 0, vitalite_tour: 0,
  },
  charge: {
    id: 'charge', name: 'Charge', icon: '/icons/charge.svg',
    vitesse: +2,
    attack_cac: +3, attack_tir: -2,
    defense_cac: -1, defense_tir: -2,
    puissance_cac: +1, puissance_tir: -1,
    intimidation_cac: +2, intimidation_tir: -1,
    courage_cac: +1, courage_tir: +1,
    esquive_cac: +1, esquive_tir: +3,
    precision_cac: +2, precision_tir: -2,
    armure: 0, armure_tour: 0, moral_tour: -10, vitalite_tour: 0,
  },
  repos: {
    id: 'repos', name: 'Repos', icon: '/icons/repos.svg',
    vitesse: -1,
    attack_cac: -2, attack_tir: -2,
    defense_cac: -2, defense_tir: -2,
    puissance_cac: -2, puissance_tir: -1,
    intimidation_cac: -2, intimidation_tir: -1,
    courage_cac: -1, courage_tir: -1,
    esquive_cac: 0, esquive_tir: -2,
    precision_cac: -1, precision_tir: -1,
    armure: 0, armure_tour: +1, moral_tour: +10, vitalite_tour: +5,
  },
  defense_combat: {
    id: 'defense_combat', name: 'Défense de combat', icon: '/icons/def_charge.svg',
    vitesse: -1,
    attack_cac: -2, attack_tir: -1,
    defense_cac: +3, defense_tir: -2,
    puissance_cac: -1, puissance_tir: 0,
    intimidation_cac: 0, intimidation_tir: +1,
    courage_cac: +1, courage_tir: -1,
    esquive_cac: +1, esquive_tir: -1,
    precision_cac: 0, precision_tir: -2,
    armure: +1, armure_tour: 0, moral_tour: 0, vitalite_tour: 0,
  },
  defense_distance: {
    id: 'defense_distance', name: 'Défense à distance', icon: '/icons/def_eparse.svg',
    vitesse: 0,
    attack_cac: -3, attack_tir: 0,
    defense_cac: -2, defense_tir: 0,
    puissance_cac: -2, puissance_tir: 0,
    intimidation_cac: -2, intimidation_tir: -1,
    courage_cac: -1, courage_tir: +1,
    esquive_cac: +2, esquive_tir: +4,
    precision_cac: -2, precision_tir: -1,
    armure: 0, armure_tour: 0, moral_tour: 0, vitalite_tour: 0,
  },
};
module.exports = STANCES;
